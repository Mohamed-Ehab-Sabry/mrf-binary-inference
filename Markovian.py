...
def all_comp(a=None, b=None, c=None):
    joint_prob = {}
    for i in range(2):
        for j in range(2):
            for k in range(2):
...
def main():
    ...
    for (a, b, c), unnorm in joint_prob.items():
        norm = normalized_prob[(a, b, c)]
        print(f"A{a} B{b} C{c}\t{unnorm}\tP(A{a},B{b},C{c}) = {norm}")

    print("\nprobability of config(1,1,1): ", query(1, 1, 1))
